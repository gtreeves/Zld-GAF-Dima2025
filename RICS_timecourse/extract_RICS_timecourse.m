...
% D2nuc_e = sqrt(var(D2nuc_full,w,3,'omitmissing'))/D;
s = w.*(D2nuc_full - D2nuc).^2;
D2nuc_e = sqrt(sum(s,3,'omitmissing')./sum(w,3,'omitmissing'))/D;
% D2nuc_e = sqrt(sum(s,3,'omitmissing')/(n_embryos-1)./mean(w,3,'omitmissing'))/D;
% D2nuc_e = std(D2nuc_full,[],3,'omitmissing')/D;
...
% Calculate S-factor
%
S_factor1 = totallsq(Inuc(:),shotnoise(:),Inuc_e(:),shotnoise_e(:));
...
for j = 1:length(varnames2)
	if exist([varnames2{j}],'var')
		data.(varnames2{j}) = eval(varnames2{j});
	end
end
...
for j = 1:length(varnames2)
	if exist([varnames2{j}],'var')
		data.([varnames2{j},'_e']) = eval([varnames2{j},'_e']);
	end
end
