...
% specify which sheet you wish to analyze (Sheet1, "tiles", or
% "tiles-additional").
%
...
%
basepth = 'G:\Shared drives\Reeveslab\Trainees\Sadia Siddika Dima\Confocal\GAF\'; % needs to be edited
filename = [basepth 'GAF.xlsx'];
% sheetname = 'Sheet1';
% sheetname = 'tiles';
sheetname = 'tiles-additional';
genotype = 'GAF-sfGFP'; % optional
outfilename = 'GAF_sfGFP_tiles-additional'; % optional
...
for iii = 1:n_embryos
	try
		...
		data = run_analyze_RICS_t(filenames(v),stages(v),istarts(v,:),iends(v,:),...
			ones(size(v)),basenames(v),[],genotype,w0,wz,sws,[],usemask,outfilename);
		Data{iii} = data;
	catch lastE
		LE{iii} = lastE;
